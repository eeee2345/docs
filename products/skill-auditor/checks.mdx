...
## 6. Code Security (Secrets + Optional SAST)

Scans all files in the skill directory for hardcoded secrets. If [Semgrep](https://semgrep.dev) is installed, also runs full SAST analysis.

**Secrets scanning (built-in):**
...
- Full static analysis via Semgrep rulesets (`p/security-audit`, `p/secrets`)
...
| SAST scanning | Not practical manually | Via Semgrep (optional) |
