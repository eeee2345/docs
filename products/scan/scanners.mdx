...
### 3. SAST via Semgrep (Optional Integration)

Runs [Semgrep](https://semgrep.dev) against source code directories when installed. Panguard does not include a built-in SAST engine -- it delegates to Semgrep for real static analysis coverage.

| Field | Details |
|-------|---------|
| **Availability** | Via `panguard scan code` subcommand (requires Semgrep installed) |
| **Prerequisite** | Install Semgrep: `pip install semgrep` or see https://semgrep.dev |
| **Rulesets used** | `p/security-audit`, `p/secrets` |
| **Output** | Findings with file path, line number, and remediation |
| **Without Semgrep** | Returns empty SAST results; secrets scanning still runs via the built-in Secrets Checker |

```bash
# Install Semgrep first
pip install semgrep

# Then run code scanning
